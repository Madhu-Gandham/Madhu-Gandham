...
public class RestaurantDAO {
	...
	public String addRestaurant(Restaurant restaurant) throws Exception   {
		...
		restaurant.setResturantId(restId);
	...
	public String checkUsers(Restaurant restaurant) {
        ...
        Session session = sessionFactory.openSession();
        Criteria cr = session.createCriteria(Restaurant.class);
        cr.add(Restrictions.eq("username", restaurant.getUsername() ));
        cr.add(Restrictions.eq("password", restaurant.getPassword()));
        List<Restaurant> listrestaurant = cr.list();
        FacesContext context=FacesContext.getCurrentInstance();
        ExternalContext externalContext=context.getExternalContext();
        
        if(listrestaurant.size()==1) {
        	externalContext.getSessionMap().put("username",restaurant.getUsername());

            return "RDashboard.jsp";
        }else {
	...
	public String idgenerate() {
		...
		if (resturantList.size() == 0) {
			...
			String id = resturantList.get(resturantList.size() - 1).getResturantId();
	...
	public String searchById(String restId) {
		...
		sessionMap.put("restaurant", res);
		return "ShowMenu.xhtml?faces-redirect=true";
		//return resturantList;
	}
